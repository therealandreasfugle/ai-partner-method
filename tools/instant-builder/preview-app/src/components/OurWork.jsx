...
// Entries with no filename are dropped rather than rendered: without the guard
// they resolved to "/work/undefined" and 404ed in the console.
const projects = brandDNA.previous_projects
  .filter((p) => p && p.filename)
  .map((p) => ({
    src: A + `/work/${p.filename}`,
    alt: p.alt,
    type: p.type,
  }));
