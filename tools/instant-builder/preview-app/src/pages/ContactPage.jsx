...
export default function ContactPage() {
  ...
  // A trade that comes to you needs the job address. A gym or a salon does not,
  // so it asks where the customer is instead, and the example follows the label.
  const addressLabel = brandDNA.copy.addressLabel || 'Property Address';
  const wantsFullAddress = /address/i.test(addressLabel);
  const addressPlaceholder = wantsFullAddress
    ? `123 Main St, ${brandDNA.address.city}, ${brandDNA.address.state}`
      ...
      >
        <div className="absolute inset-0 w-full h-full" style={{ zIndex: 1 }}>
          <img
            ...
            onError={(e) => { e.target.src = A + '/work/project1.webp'; }}
          />
        ...
        <div className="max-w-7xl mx-auto px-8 grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2">
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-xs font-bold text-cool uppercase tracking-wider mb-1.5">{addressLabel}</label>
                <input className="form-input w-full px-4 py-3 text-sm placeholder-white/40" style={glassInput} placeholder={addressPlaceholder} />
              </div>
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    {/* Built from whatever parts exist. Printing every field
                        blind left a leading comma and an empty first line when
                        the scrape only knew the town. */}
                    <div className="font-heading font-bold text-white text-sm leading-snug">
                      {[brandDNA.address.street,
                        [brandDNA.address.city, brandDNA.address.state].filter(Boolean).join(', '),
                        brandDNA.address.zip]
                        .filter(Boolean)
