...
import { A } from '../config/asset-base'
...
function HeroBackground({ position = '50% 30%', overlayTop = 0.55, overlayMid = 0.45, overlayBottom = 0.75 }) {
  return (
    <div className="absolute inset-0 w-full h-full" style={{ zIndex: 1 }}>
      <picture>
        <source media="(max-width: 768px)" srcSet={A + "/hero-image-mobile.webp"} type="image/webp" />
        <source srcSet={A + "/hero-image.webp"} type="image/webp" />
        <img
          src={A + "/hero-image.webp"}
...
function RatingPills({ align = 'left' }) {
  // A generated preview has no review data, because inventing a rating for
  // someone else's business is not on. Rendering the pills anyway printed
  // "0.0 (0)" next to five gold stars, which reads worse than showing nothing.
  // Real clients with real numbers are unaffected.
  const reviews = brandDNA.reviews || {}
  if (!reviews.rating || !(reviews.googleCount || reviews.facebookCount)) return null

  // Each platform is shown ONLY if it has real reviews. A scraped Google
  // listing gives us a genuine Google count but nothing for Facebook, and
  // rendering the Facebook pill anyway printed "4.8 (0)", which reads as a
  // business with no reputation rather than one with none listed there.
  const showGoogle = reviews.googleCount > 0
  const showFacebook = reviews.facebookCount > 0
  const single = showGoogle !== showFacebook
  ...
  return (
    <div className={`grid ${single ? 'grid-cols-1 max-w-[220px]' : 'grid-cols-2 max-w-[400px]'} gap-[8px] mb-6 lg:max-w-none lg:flex lg:items-center lg:gap-2 lg:flex-wrap ${align === 'center' ? 'mx-auto lg:mx-0 lg:justify-center' : 'lg:justify-start'}`}>
      {showGoogle && (
